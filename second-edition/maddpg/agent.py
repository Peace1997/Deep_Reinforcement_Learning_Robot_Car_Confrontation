...
from maddpg.model import CentralizedCritic, Actor
...
class DDPGAgent:
    ...
    def __init__(self, env, agent_id, actor_lr=1e-4, critic_lr=1e-3, gamma=0.99, tau=1e-2):
        ...
        self.num_agents = self.env.red_agent_num

        self.critic_input_dim = int(np.sum([env.observation_space[agent].shape[0] for agent in range(env.red_agent_num)]))
    ...
    def update(self, indiv_reward_batch, indiv_obs_batch, global_state_batch, global_actions_batch, global_next_state_batch, next_global_actions,writer,episode):
        """
        ...
        writer.add_scalar('Loss Q', critic_loss, episode)
        ...
        writer.add_scalar('Loss P', policy_loss, episode)
    ...
    def save_ID_R1(self,filename):
        torch.save(self.actor.state_dict(),filename+ 'actor1.pkl')
        torch.save(self.critic.state_dict(), filename+ 'critic1.pkl')
    def save_ID_R2(self,filename):
        torch.save(self.actor.state_dict(),filename+ 'actor2.pkl')
        torch.save(self.critic.state_dict(), filename+ 'critic2.pkl') 
    
    def red_load(self):
        self.actor.load_state_dict(torch.load('model2_0_1/5/actor.pkl'))
        self.critic.load_state_dict(torch.load('model2_0_1/5/critic.pkl'))
